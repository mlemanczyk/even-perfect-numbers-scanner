...
public sealed class DeviceDependentGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateForTemplate(SourceProductionContext context, TemplateInfo template, Compilation compilation, GeneratorOptions options)
    {
        ...
        foreach (TemplateVariant variant in template.ExpandVariants())
        {
            ...
            string hintName = options.BuildHintName(template.ContainingNamespace, generatedFileName);
            context.AddSource(
                hintName,
    ...
    private readonly struct GeneratorOptions
    {
        ...
        private const string GeneratedRootOptionName = "build_property.DeviceDependentGenerator_OutputRoot";
        ...
        public string BuildHintName(string containingNamespace, string generatedFileName)
        {
            string root = GetGeneratedRoot();
            string trimmedRoot = string.IsNullOrWhiteSpace(root)
                ? GeneratedDirectoryName
                : root.Replace('\\', '/').Trim('/').Trim();
            if (string.IsNullOrWhiteSpace(trimmedRoot))
            {
                trimmedRoot = GeneratedDirectoryName;
            }

            string namespacePath = string.IsNullOrWhiteSpace(containingNamespace)
                ? string.Empty
                : containingNamespace.Replace('.', '/').Trim('/');

            if (namespacePath.Length == 0)
            {
                return trimmedRoot + "/" + generatedFileName;
            }

            return trimmedRoot + "/" + namespacePath + "/" + generatedFileName;
        }

        private string GetGeneratedRoot()
        {
            if (globalOptions.TryGetValue(GeneratedRootOptionName, out string? configuredRoot)
