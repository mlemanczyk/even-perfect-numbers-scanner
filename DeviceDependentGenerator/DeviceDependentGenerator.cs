...
public sealed class DeviceDependentGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(templates.Combine(options), static (productionContext, pair) =>
        {
            GenerateForTemplate(productionContext, pair.Left, pair.Right);
        });
    }
    ...
    private static void GenerateForTemplate(SourceProductionContext context, TemplateInfo template, GeneratorOptions options)
    {
        ...
        foreach (string enumValueName in template.EnumValues)
        {
            ...
            context.AddSource(hintName, SourceText.From(generatedRoot.ToFullString(), Encoding.UTF8));
        }
    }
