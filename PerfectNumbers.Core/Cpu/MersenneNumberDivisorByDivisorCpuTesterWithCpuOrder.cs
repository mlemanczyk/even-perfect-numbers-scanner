...
public struct MersenneNumberDivisorByDivisorCpuTesterWithCpuOrder() : IMersenneNumberDivisorByDivisorTester
{
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	public readonly IMersenneNumberDivisorByDivisorTester.IDivisorScanSession CreateDivisorSession(PrimeOrderCalculatorAccelerator gpu) => new MersenneCpuDivisorScanSessionWithCpuOrder();
		...
		out ulong foundDivisor)
	{
		...
		while (divisor.CompareTo(limit) <= 0)
		{
			if (residueStepper.IsAdmissible())
			{
				...
				if (!MersenneNumberDivisorByDivisorTester.TryCalculateCycleLengthForExponentCpu(
						candidate,
						prime,
						divisorData,
				...
				RecordState(StateFilePath, currentK);
	...
	private void RecordState(in string path, BigInteger k)
	{
