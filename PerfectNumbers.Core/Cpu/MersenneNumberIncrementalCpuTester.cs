    ...
    public void Scan(ulong exponent, UInt128 twoP, bool lastIsSeven, UInt128 maxK, ref bool isPrime)
    {
        ...
        while (k <= maxK && Volatile.Read(ref isPrime))
        {
            ...
            if (shouldCheck)
            {
                ...
                {
                    reject = exponent.Pow2ModWindowed(q, qCycle) == UInt128.One;
                }
                else
                {
                    phi = q - one;
                    ...
                    {
                        phi64 = (ulong)phi;
                        if (phi64.Pow2ModWindowed(q, qCycle) == one)
                        {
                            ulong halfExponent = phi64 >> 1;
                            halfPow = halfExponent.Pow2ModWindowed(q, qCycle) - one;
                            if (halfPow.BinaryGcd(q) == one)
                            {
                                div = phi64.FastDiv64(exponent, divMul);
                                divPow = div.Pow2ModWindowed(q, qCycle) - one;
