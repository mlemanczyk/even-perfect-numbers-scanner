...
public sealed class ModResidueTracker
{
	...
	public bool IsDivisibleCpu(UInt128 number, UInt128 divisor)
	{
		ArgumentOutOfRangeException.ThrowIfEqual(divisor, UInt128.Zero);
		...
		{
			UInt128 residue = ComputeInitialResidueCpu(number, divisor);
	...
	/// divisors on first use.
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public bool IsDivisibleGpu(PrimeOrderCalculatorAccelerator gpu, UInt128 number, UInt128 divisor)
	{
		ArgumentOutOfRangeException.ThrowIfEqual(divisor, UInt128.Zero);
		...
		if (idx == divisors.Count || divisors[idx] != divisor)
		{
			UInt128 residue = ComputeInitialResidueGpu(gpu, number, divisor);
			divisors.Insert(idx, divisor);
			_residues.Insert(idx, residue);
			if (_hasLastAppended && idx == divisors.Count - 1 && divisor >= _lastAppended)
			{
	...
	/// divisors on first use.
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public bool IsDivisibleHybrid(PrimeOrderCalculatorAccelerator gpu, UInt128 number, UInt128 divisor)
	{
		ArgumentOutOfRangeException.ThrowIfEqual(divisor, UInt128.Zero);
		...
		if (idx == divisors.Count || divisors[idx] != divisor)
		{
			UInt128 residue = ComputeInitialResidueHybrid(gpu, number, divisor);
			divisors.Insert(idx, divisor);
			_residues.Insert(idx, residue);
			if (_hasLastAppended && idx == divisors.Count - 1 && divisor >= _lastAppended)
			{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public bool MergeOrAppendCpu(UInt128 number, UInt128 divisor, out bool divisible)
	{
		...
		{
			residue = ComputeInitialResidueCpu(number, divisor);
	...
	// Returns true and sets 'divisible' based on residue at 'number'.
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public bool MergeOrAppendGpu(PrimeOrderCalculatorAccelerator gpu, UInt128 number, UInt128 divisor, out bool divisible)
	{
		// Assume number is non-decreasing; advance residues lazily when number changes.
		List<UInt128> divisors = _divisors;
		if (number != _currentNumber)
		...
		if (divisors.Count == 0 || (_hasLastAppended && divisor >= _lastAppended && divisors[^1] <= divisor))
		{
			residue = ComputeInitialResidueGpu(gpu, number, divisor);
			divisors.Add(divisor);
			residues.Add(residue);
			_hasLastAppended = true;
			_lastAppended = divisor;
	...
	// Returns true and sets 'divisible' based on residue at 'number'.
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public bool MergeOrAppendHybrid(PrimeOrderCalculatorAccelerator gpu, UInt128 number, UInt128 divisor, out bool divisible)
	{
		// Assume number is non-decreasing; advance residues lazily when number changes.
		List<UInt128> divisors = _divisors;
		if (number != _currentNumber)
		...
		if (divisors.Count == 0 || (_hasLastAppended && divisor >= _lastAppended && divisors[^1] <= divisor))
		{
			residue = ComputeInitialResidueHybrid(gpu, number, divisor);
			divisors.Add(divisor);
			residues.Add(residue);
			_hasLastAppended = true;
			_lastAppended = divisor;
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private UInt128 ComputeInitialResidueCpu(UInt128 number, UInt128 divisor)
	{
                if (_model == ResidueModel.Identity)
                {
		...
		ulong ord = divisor.CalculateOrderCpu();
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private UInt128 ComputeInitialResidueGpu(PrimeOrderCalculatorAccelerator gpu, UInt128 number, UInt128 divisor)
	{
                if (_model == ResidueModel.Identity)
                {
                        // Fast path for 64-bit inputs
		...
		ulong ord = divisor.CalculateOrderGpu(gpu);
		if (ord == 0UL || number % (UInt128)ord != UInt128.Zero)
			return UInt128.One; // does NOT divide
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private UInt128 ComputeInitialResidueHybrid(PrimeOrderCalculatorAccelerator gpu, UInt128 number, UInt128 divisor)
	{
                if (_model == ResidueModel.Identity)
                {
                        // Fast path for 64-bit inputs
		...
		ulong ord = divisor.CalculateOrderHybrid(gpu);
		if (ord == 0UL || number % (UInt128)ord != UInt128.Zero)
			return UInt128.One; // does NOT divide
