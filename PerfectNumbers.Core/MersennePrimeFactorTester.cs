...
public static class MersennePrimeFactorTester
{
	...
	private static readonly ConcurrentFixedCapacityStack<List<ulong>> ListPool = new(PerfectNumberConstants.DefaultPoolCapacity);
	private static readonly ConcurrentFixedCapacityStack<Dictionary<ulong, byte>> DictPool = new(PerfectNumberConstants.DefaultPoolCapacity);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static List<ulong> RentList() => ListPool.Pop() is { } list ? list : new List<ulong>(4);
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static Dictionary<ulong, byte> RentDictionary() => DictPool.Pop() is { } dict ? dict : [];
	...
	private static readonly ConcurrentFixedCapacityStack<Dictionary<UInt128, byte>> DictPool128 = new(PerfectNumberConstants.DefaultPoolCapacity);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static Dictionary<UInt128, byte> RentDictionary128() =>
	DictPool128.Pop() is { } dict ? dict : [];

		[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static void ReturnDictionary128(Dictionary<UInt128, byte> dict)
	{
		dict.Clear();
		DictPool128.Push(dict);
	}
	...
	private static ulong[] Factor64(ulong n, CancellationToken ct)
	{
		...
		var queue = ThreadStaticPools.RentUlongStack(PerfectNumberConstants.DefaultPoolCapacity); // TODO: Replace this Queue with the pooled stack from the
		...
		ThreadStaticPools.ReturnUlongStack(queue);
