...
{
	private readonly struct WorkItem(Task task)
	{
	...
	public void Queue(Task task)
	{
		...
		lock (_lock)
		{
			ObjectDisposedException.ThrowIf(_disposeCancellation.IsCancellationRequested, nameof(TaskThreadPool));

			_pendingTaskQueue.Push(new WorkItem(task));
			_pendingTaskCount++;
	...
	public IEnumerable<Task> GetScheduledTasks()
	{
		...
		ConcurrentFixedCapacityStack<WorkItem>.Return(snapshot);
		return tasks;
	}
	...
	public void Dispose()
	{
		...
		lock (_lock)
		{
			...
			while (_pendingTaskQueue.Pop() is { })
			{
			}
