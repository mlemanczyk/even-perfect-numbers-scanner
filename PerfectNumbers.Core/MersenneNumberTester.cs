	...
	ComputationDevice orderDevice = ComputationDevice.Gpu)
{
	...
	public void WarmUpOrdersHybrid(ulong exponent, ulong limit = 5_000_000UL)
	{
		...
		if (!useGpuOrder)
		{
			...
			for (; k <= limit; k++)
			{
				...
				ulong ord = q.CalculateOrderHybrid(gpu);
				_orderCache[q] = ord;
			}
		...
		while (offset < idx)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (order == 0UL)
				{
					order = qs[offset + i].CalculateOrderHybrid(gpu);
				}
		...
		PrimeOrderCalculatorAccelerator.Return(gpu);
		ThreadStaticPools.UInt128Pool.Return(qs);
	}

	public void WarmUpOrdersCpu(ulong exponent, ulong limit = 5_000_000UL)
	{
		bool cacheEnabled = useOrderCache;
		if (!cacheEnabled)
		{
		...
		while (offset < idx)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (order == 0UL)
				{
					order = qs[offset + i].CalculateOrderCpu();
				}
	...
	public void WarmUpOrdersGpu(ulong exponent, ulong limit = 5_000_000UL)
	{
		bool cacheEnabled = useOrderCache;
		if (!cacheEnabled)
		{
		...
		if (!useGpuOrder)
		{
			...
			for (; k <= limit; k++)
			{
				...
				ulong ord = q.CalculateOrderGpu(gpu);
				_orderCache[q] = ord;
			}
