...
public static class ULongExtensions
{
	...
	public static ulong MulHigh(this ulong x, ulong y)
	{
		...
		result = (xHigh * yHigh) + (w1 >> 32) + (w2 >> 32);
		result += ((xLow >> 32) + (uint)w1 + (uint)w2) >> 32;
		return result;
	}
	...
	public static ulong MontgomeryMultiply(this ulong a, ulong b, ulong modulus, ulong nPrime)
	{
		...
		result = unchecked(a.MulHigh(b) + m.MulHigh(modulus) + (unchecked(tLow + result) < tLow ? 1UL : 0UL));
		if (result >= modulus)
		{
			result -= modulus;
		}
	...
	internal static ulong Pow2MontgomeryModWindowedKernel(ulong exponent, in MontgomeryDivisorData divisor, bool keepMontgomery)
	{
		...
		while (index >= 0)
		{
			...
			int windowStart = index - windowSize + 1;
			// This can never happen in production code
			// if (windowStart < 0)
			// {
			//     windowStart = 0;
			// }
			...
			bitLength = index - windowStart + 1;
			for (int square = 0; square < bitLength; square++)
			{
				result = result.MontgomeryMultiply(result, modulus, nPrime);
			}

			ulong windowValue = (1UL << bitLength) - 1UL;
			windowValue = (exponent >> windowStart) & windowValue;
			windowValue = ComputeMontgomeryOddPower(windowValue, divisor, modulus, nPrime);
			result = result.MontgomeryMultiply(windowValue, modulus, nPrime);
	...
	private static class Pow2MontgomeryGpuExecutor
	{
		...
		public static bool TryExecute(ulong exponent, in MontgomeryDivisorData divisor, bool keepMontgomery, out ulong result)
		{
			...
			// We could clear the buffer, but it'll be overwritten anyway
	...
	private static int GetWindowSize(int bitLength) => bitLength switch
	{
		<= 6 => bitLength,// bitLength is always >= 1 in production code
