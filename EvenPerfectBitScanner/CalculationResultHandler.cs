...
// TODO: Convert this struct to an equivalent of a static class. Make all fields / properties / methods static and initialize it during program startup. Don't check if it's initialized within. Rely on the flow enforcing it.
internal struct CalculationResultHandler
{
	...
	private readonly bool IsInitialized => _sync is not null;
	private readonly bool HasFileSync => _fileWriteSync is not null;
	...
	internal readonly string ResultsFileName => _resultsFileName;
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private readonly int TryFormatResultLine(Span<byte> destination, ulong currentP, bool searchedMersenne, bool detailedCheck, bool passedAllTests)
	{
		int offset = 0;
